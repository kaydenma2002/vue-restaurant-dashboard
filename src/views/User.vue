<template>
  <div>
    <v-container>
      ...
      <v-row>
        <v-col cols="cols">
          <panelComponentVue icon="mdi-account" title="Admins" :value="this.data?.admin?.length"></panelComponentVue>
        </v-col>
        <v-col cols="cols">
          <panelComponentVue icon="mdi-cart" title="Products" :value="this.data?.restaurants?.length"></panelComponentVue>
        </v-col>

        <v-col cols="cols">
          <panelComponentVue icon="mdi-earth" title="Visitors" :value="this.data?.customers?.length"></panelComponentVue>
        </v-col>
      </v-row>
...
export default {
  ...
  created() {
    ...
    this.channel = this.echo.channel('user')
    this.channel.listen('UserCreated', (data) => {
      Swal.fire({
        position: 'top-end',
        icon: 'success',
