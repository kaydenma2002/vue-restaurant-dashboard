    ...
    <tbody>
      <tr v-for="items in paginatedData" :key="items.id">
        ...
        <td class="flex justify-space-between">
          <v-btn size="small" color="primary" @click.prevent="view(items.id)"
            >View</v-btn
          >
          <v-btn size="small" color="warning" @click.prevent="deletes(items.id)"
            >Delete</v-btn
          >
        </td>
      </tr>
      ...
      HTTPS.post("admin/viewOrderById", {
        ...
        Swal.fire({
          ...
          html:
            ...
            `<div class="text-left mb-2">Item:  ${res.data[1]
